...
public class MarieEditor extends JFrame {
  /******************************************************************************************
   * This program provides simple editing functions for programs that will be used
   * in * the MARIE machine simulator environment. The MARIE assembler can be
   * invoked * through a menu pick on the main menu bar. If the assembler finds
   * errors, the user * is notified and a frame displays showing the assembler
   * listing. The listing may * also be summoned through a menu option. * * This
   * program can be invoked in any of three ways: * * 1. Through a console
   * command: * * java MarieEditor * * 2. Through a call to the static method: * *
   * editNewFile (boolean exitMode) * * where exitMode indicates whether the
   * program should terminate * (System.exit(0)) or just return to its caller. * *
   * 3. Through a call to the static method: * * editFile(String fileToEdit,
   * boolean exitMode) * * where exitMode has the same meaning as above. * * For
   * opening and saving files, we have set the default file filter to look for *
   * MARIE assembly code files in the user's current directory. Any text file in
   * any * directory to which the user has authority may be edited using this
   * editor. * * This program employs a JEditorPane instead of a JTextArea because
   * of the objection- * able scroll pane behavior of the latter. Because of this,
   * we have to create our * own document listener as the Java DocumentListener is
   * not applicable to JEditorPane. * * If the user selects the File | Exit menu
   * option, a wondow-closing event is * dispatched so that the caller can monitor
   * the event and destroy the editor's * instance if desired. * *
   ******************************************************************************************/
  ...
  public static final JFileChooser sourceFileChooser = new JFileChooser(System.getProperty("user.dir")); // Filters for
                                                                                                         // loading
  ...
  class MarieSourceFileFilter extends javax.swing.filechooser.FileFilter {
    /******************************************************************************************
     * Inner class makes sure we can load only file types relevant to editor
     * processing, * assembler files in particular. This filter can be overridden by
     * user-selection of * the wildcard option from the file chooser box. *
     ******************************************************************************************/
  ...
  public MarieEditor(boolean exitMode) {
    /******************************************************************************************
     * This constructor sets the exit mode (whether to just close the program or do
     * a * System.exit) and calls the method to build the GUI interface. *
     ******************************************************************************************/
  ...
  public MarieEditor(String fileToEdit, boolean exitMode) {
    /******************************************************************************************
     * This constructor sets the exit mode (whether to just close the program or do
     * a System. * exit) saves a default filename and loads the file after building
     * the GUI screen. *
     ******************************************************************************************/
    exitOnClose = exitMode;
    initializeScreen();
  ...
  private void initializeScreen() {
    /******************************************************************************************
     * Sets the appearance an behavior of the JFrame, and the menu. Adds listeners
     * to each * button. The order in which the components are created is the order
     * in which they * appear on the screen starting from left to right. *
     ******************************************************************************************/
    setIconImage(Toolkit.getDefaultToolkit().createImage(MarieEditor.class // Set JFrame appearance
    ...
    setLocation((screenSize.width - myFrameSize.width) / 2, (screenSize.height - myFrameSize.height) / 2);
    setVisible(true);
  ...
  void setButtonsForModifiedFile() {
    /******************************************************************************************
     * Set menu buttons so that they make sense when we have sensed a substantive
     * change * to the document area. *
     ******************************************************************************************/
    newFileItem.setEnabled(true);
    closeFileItem.setEnabled(true);
  ...
  void openFile() {
    /******************************************************************************************
     * Identifies a file to load from disk, but first checks whether a
     * currently-visible * document has been modified and whether those
     * modifications can be abandoned. If * the user decides not to abort the
     * document, this method exits. * * Next, a JFileChooser appears to allow
     * selection of a file from disk. This chooser * has been set so that the
     * current directory is interrogated and only assembler code * source files
     * appear in the list. The user can override this and load any other type * of
     * plain text file. * * If the JFileChooser.APPROVE_OPTION isn't true, nothing
     * happens. (The user has * canceled.) If it is true, we store the name of the
     * file in the variable currFileName * and attempt to load the selected file
     * from disk. *
     ******************************************************************************************/
    if (!abandonOkay()) {
      return;
  ...
  void getFile() {
    /******************************************************************************************
     * Loads a file named currFileName from disk into the document area of the
     * editor. * If the selected file is assembler source code, we also look for an
     * assembler listing. *
     ******************************************************************************************/
  ...
  boolean listingAvailable() {
     ...
     * * currFileName. If we find it, we simply enable the menu button that displays
     * it. * Note: currFileName has an ASSEMBER_FILE_TYPE extension, so we have to
     * parse it to * retrieve the root file name to which a LISTING_FILE_TYPE
     * extension is added before * we look for the file. *
     ******************************************************************************************/
    try {
    ...
    if (currFilePrefix == null)
      return false;
    try {
      InputStream fileIn = new FileInputStream(currFilePath + currFilePrefix + LISTING_FILE_TYPE);
  ...
  void newFile() {
    /******************************************************************************************
     * Clears the input area and resets the filename. If modified text is in the
     * document * area, the user is first asked to provide confirmation and given
     * the opportunity to * save the modified text. *
     ******************************************************************************************/
    if (abandonOkay()) {
      sourceCodeArea.setText("");
      setButtonsForNewFile();
  ...
  boolean saveFile() {
    /******************************************************************************************
     * Saves the current file if its name isn't null. If the file has the extension
     * of an * assembly-language file, we activate buttons to permit assembly of the
     * file. If the * filename is null, we invoke the "save-as" method. Whether the
     * save is successful or * unsuccessful, the user is given feedback in the
     * message area. *
     ******************************************************************************************/
    if (currFileName == null) {
      return saveAsFile();
  ...
  boolean abandonOkay() {
    /******************************************************************************************
     * If the text currently in the pane has been modified, display an option pane
     * asking * the user to confirm that the updates can be abandoned. *
     ******************************************************************************************/
    if (!fileUpdated) {
      return true;
    }
  ...
  boolean saveAsFile() {
    /******************************************************************************************
     * Allows user selection or key entry of a file name to use for saving current
     * edits. * If the file exists, the overlay confirmation method is invoked. *
     ******************************************************************************************/
  ...
  boolean overlayOkay() {
    /******************************************************************************************
     * Returns true or false to confirm overlay of a file, based on value returned
     * from * a JOptionPane. *
     ******************************************************************************************/
    ...
    int option = JOptionPane.showOptionDialog(closingFrame, "Rewrite " + currFileName + "?", "Rewrite Confirmation",
  ...
  String createFileMsg() {
    /******************************************************************************************
     * Creates a string containing the state of the currently-loaded file. This
     * message is * displayed through the showMessage() method below. *
     ******************************************************************************************/
    String message;
    if (currFileName == null) {
      message = "Untitled";
  ...
  void showMessage(int severity, String message) {
    /******************************************************************************************
     * Places the parameter string in the message (text) field at the bottom of the
     * editor * in a color appropriate to the severity code. *
     ******************************************************************************************/
    switch (severity) {
    case 1:
  ...
  void assembleFile() {
    /******************************************************************************************
     * Saves the current edits (if needed) and invokes the Assembler's static
     * assembleFile * method, which returns the total number of errors found during
     * assembly. This method * then uses this error count to display a status
     * message. If errors were detected * during assembly, the showListing() method
     * is invoked. *
     ******************************************************************************************/
  ...
  void showListing() {
    /******************************************************************************************
     * Uses the root filename of the current file concatenated with a listing suffix
     * to * create an instance of the TextFileViewer. The TextFileViewer constructor
     * is passed * a string to place in the title of its frame, the filename to
     * display and a boolean * that indicates whether TextFileViewer should call a
     * System.exit(0) when it terminates. *
     ******************************************************************************************/
    ...
    try {
      ...
      listingFileViewer = new TextFileViewer("Assembly Listing for " + currFilePrefix + ASSEMBLER_FILE_TYPE,
          currFilePath + currFilePrefix + LISTING_FILE_TYPE, false);

      Dimension myFrameSize = this.getSize();
      Dimension tfvSize = listingFileViewer.getPreferredSize();
      Point location = getLocation();
      listingFileViewer.setLocation(((myFrameSize.width + (location.x / 3)) / 2), (location.y / 3));
      listingFileViewer.show();
    }
  } // showListing()
  ...
  void editHelp() {
    /******************************************************************************************
     * Create an instance of the TextFileViewer to display the help file. We pass a
     * string * to be used for the frame title, the filename to display and a
     * boolean that indicates * whether TextFileViewer should call a System.exit(0)
     * when it terminates. *
     ******************************************************************************************/
    try {
      helpViewer01.show();
  ...
  public static void editNewFile(boolean exitMode) {
    /******************************************************************************************
     * Static method is callable from other objects. Creates an instance of the
     * editor with * no file loaded. (A blank text pane.) *
     ******************************************************************************************/
  ...
  public static void editFile(String fileToEdit, boolean exitMode) { // This method is invoked when called from outside.
    /******************************************************************************************
     * Same as editNewFile() except it invokes a constructor that loads the file
     * given in * the parameter list (If it can be found.) *
     ******************************************************************************************/
  ...
  /** Main method */
  /******************************************************************************************
   * The presence of this main() method permits this program to be used in
   * "console mode." *
   ******************************************************************************************/
